...
def scale_xy(img, x, y):
    return cv2.resize(img, ( int(img.shape[0] * x) , int(img.shape[1] * y) ), interpolation=cv2.INTER_NEAREST_EXACT)
...
class Smolbot:
    ...
    def render(self, traits, face_w, face_h, face_x, face_y, capture_w, capture_h):
        ...
        body = blit(body, scale_xy(self.sprite_mouth_1, 90, self.mouth_opening * 10), (self.mouth_x, self.mouth_y), centered=True)

        buffer = blit(buffer, rotate(scale(body, self.scale), -deg), (self.body_x, self.body_y), centered=True)

        cv2.imshow("Avatar view", buffer)

        return buffer

    def open_mouth(self, val):
        self.mouth_opening = 1 + (val // MAX_MOUTH_OPENING)
...
def get_sound(indata, outdata, frames, time, status):
    ...
    smolbot.open_mouth(volume_norm)
...
def main():
    ...
    delta = 0
    with pyvirtualcam.Camera(width=BUFFER_COLS, height=BUFFER_ROWS, fps=30) as cam, sd.Stream(callback=get_sound):
